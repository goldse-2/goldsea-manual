...
<head>
    ...
    <style>
        ...
        /* 动态表格行悬停色 */
        .theme-paojiao .clickable-row:hover { background-color: #eff6ff; }
        .theme-fanqie .clickable-row:hover { background-color: #fef2f2; }
        .theme-shajiang .clickable-row:hover { background-color: #fffbeb; }
        .theme-nangua .clickable-row:hover { background-color: #fff7ed; }
        .theme-nongjia .clickable-row:hover { background-color: #ecfdf5; }
        .theme-chaoshan .clickable-row:hover { background-color: #f0fdfa; }
        .theme-hunan .clickable-row:hover { background-color: #fff1f2; }
        .theme-kejia .clickable-row:hover { background-color: #e0e7ff; }
        .theme-hubei .clickable-row:hover { background-color: #cffafe; }
        .theme-dongbei .clickable-row:hover { background-color: #f3e8ff; }
        .theme-beef .clickable-row:hover { background-color: #fdf4ff; }
        .theme-pork .clickable-row:hover { background-color: #fdf2f8; }
        ...
        .modal-title-overlay {
            position: absolute; bottom: 1.25rem; left: 1.5rem; color: white; z-index: 10;
            font-size: 1.75rem; font-weight: bold; text-shadow: 0 2px 4px rgba(0,0,0,0.5);
        }
        .modal-close-float {
        ...
        .step-circle {
            ...
            width: 1.5rem; height: 1.5rem; color: white; border-radius: 9999px;
...
<div class="max-w-6xl mx-auto relative">
    <div class="mb-8 text-center">
        <h1 class="text-4xl font-extrabold text-gray-800 mb-4 tracking-tight">270道精选家常菜谱大全</h1>
        <p class="text-gray-600 max-w-2xl mx-auto mb-6">收录十二大风味菜系及特色专题，共270道绝味家常菜。解决您“每天吃什么”的终极难题！</p>
        
        <!-- 抽奖大按钮 -->
        <button onclick="openLuckyDraw()" class="mb-8 bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white font-bold py-3 px-8 rounded-full shadow-lg transform transition hover:scale-105 flex items-center justify-center mx-auto text-lg">
        ...
        <div class="flex flex-col md:flex-row justify-center items-center mb-8 space-y-4 md:space-y-0 md:space-x-4">
            <div class="relative w-full max-w-sm">
                <select id="category-select" onchange="switchCategory(this.value)" class="block appearance-none w-full bg-white border-2 border-blue-200 hover:border-blue-400 px-4 py-3 pr-8 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-700 text-lg font-bold text-center cursor-pointer transition-colors">
                    ...
                    <option value="hunan">湖南菜 (20道)</option>
                    <option value="kejia">客家菜 (40道)</option>
                    <option value="hubei">湖北菜 (20道)</option>
                    <option value="dongbei">东北菜 (20道)</option>
            ...
            <!-- 搜索框 -->
            <div class="relative w-full max-w-sm">
                <input type="text" id="search-input" oninput="handleSearch()" placeholder="🔍 搜索菜名、食材..." class="block w-full bg-white border-2 border-gray-200 hover:border-blue-400 px-4 py-3 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-700 text-lg transition-colors">
            </div>
        </div>
    </div>

    <!-- 表格区域 -->
    <div id="table-wrapper" class="table-container overflow-x-auto theme-paojiao">
...
<div id="recipe-modal" class="modal-overlay" onclick="closeModalOnOutsideClick(event, 'recipe-modal')">
    <div class="modal-content relative p-0" id="recipe-modal-content">
        ...
        <div class="p-6 md:p-8">
            ...
            <div>
                ...
                <div id="modal-steps" class="timeline-container space-y-6"></div>
            </div>
        </div>
    </div>
</div>
...
<div id="lucky-draw-modal" class="modal-overlay" onclick="closeModalOnOutsideClick(event, 'lucky-draw-modal')">
    <div class="modal-content relative" id="lucky-modal-content">
        ...
        <h2 class="text-3xl font-black text-red-600 drop-shadow-sm mb-2">今天吃什么？</h2>
        <p class="text-red-400 font-bold mb-4">从全库精选好菜中随机抽取</p>
        ...
        <!-- 中奖结果展示区 -->
        <div id="winner-result" class="winner-card bg-white p-4 rounded-xl shadow-lg mt-4 border-2 border-red-100">
            <p class="text-gray-500 font-bold mb-2">🎉 抽中啦！今天就吃：</p>
            <img id="winner-img" src="" class="w-full h-40 object-cover rounded-lg mb-3 shadow-inner" alt="winner">
            <h3 id="winner-name" class="text-2xl font-bold text-gray-800 mb-3"></h3>
            <button id="view-winner-btn" class="w-full bg-red-50 text-red-600 hover:bg-red-100 font-bold py-2 rounded-lg border border-red-200 transition">
                查看做法详情
            </button>
        </div>
    </div>
...
<script>
        ...
        if (extraNamesMap[cat]) {
            extraNamesMap[cat].forEach(extraDataStr => {
                ...
                expandedDatabase[cat].push({
                    name: name,
                    desc: desc,
                    image: `https://tse1-mm.cn.bing.net/th?q=${encodeURIComponent(name)}+家常菜美食实拍&w=600&h=400&c=7&rs=1&p=0&mkt=zh-CN`,
                    ingredients: ingredientsRaw,
                    steps: [
                        { time: "5分钟", desc: `将主要的食材如 ${mainIngredient} 等清洗干净，改刀切成合适的形状备用。准备好葱姜蒜等基础香辛料。` },
                        { time: "3分钟", desc: "热锅起油，下入葱姜蒜等香料爆出香味，激发底层风味。" },
                        { time: "10-15分钟", desc: `下入 ${mainIngredient} 及其他配菜大火翻炒或中小火慢炖，加入菜系特色的调味料（如生抽、盐等）翻拌均匀，焖煮入味。` },
    ...
    function deleteRecipe(category, index, event) {
        event.stopPropagation(); // 阻止行点击事件（不弹窗）
    ...
    // UI 主题配置
    const themes = {
        paojiao: { tableHeadBg: 'bg-blue-50', tableHeadText: 'text-blue-800', tableHeadBorder: 'border-blue-100', btnClass: 'bg-blue-600', modalIconBg: 'bg-blue-500', tagBg: 'bg-blue-50', tagText: 'text-blue-700', tagBorder: 'border-blue-100', timeBg: 'bg-blue-50', timeText: 'text-blue-600' },
        fanqie: { tableHeadBg: 'bg-red-50', tableHeadText: 'text-red-800', tableHeadBorder: 'border-red-100', btnClass: 'bg-red-500', modalIconBg: 'bg-red-500', tagBg: 'bg-red-50', tagText: 'text-red-700', tagBorder: 'border-red-100', timeBg: 'bg-red-50', timeText: 'text-red-600' },
        shajiang: { tableHeadBg: 'bg-amber-50', tableHeadText: 'text-amber-800', tableHeadBorder: 'border-amber-100', btnClass: 'bg-amber-500', modalIconBg: 'bg-amber-500', tagBg: 'bg-amber-50', tagText: 'text-amber-700', tagBorder: 'border-amber-100', timeBg: 'bg-amber-50', timeText: 'text-amber-600' },
        nangua: { tableHeadBg: 'bg-orange-50', tableHeadText: 'text-orange-800', tableHeadBorder: 'border-orange-100', btnClass: 'bg-orange-500', modalIconBg: 'bg-orange-500', tagBg: 'bg-orange-50', tagText: 'text-orange-700', tagBorder: 'border-orange-100', timeBg: 'bg-orange-50', timeText: 'text-orange-600' },
        nongjia: { tableHeadBg: 'bg-emerald-50', tableHeadText: 'text-emerald-800', tableHeadBorder: 'border-emerald-100', btnClass: 'bg-emerald-500', modalIconBg: 'bg-emerald-500', tagBg: 'bg-emerald-50', tagText: 'text-emerald-700', tagBorder: 'border-emerald-100', timeBg: 'bg-emerald-50', timeText: 'text-emerald-600' },
        chaoshan: { tableHeadBg: 'bg-teal-50', tableHeadText: 'text-teal-800', tableHeadBorder: 'border-teal-100', btnClass: 'bg-teal-500', modalIconBg: 'bg-teal-500', tagBg: 'bg-teal-50', tagText: 'text-teal-700', tagBorder: 'border-teal-100', timeBg: 'bg-teal-50', timeText: 'text-teal-600' },
        hunan: { tableHeadBg: 'bg-rose-50', tableHeadText: 'text-rose-800', tableHeadBorder: 'border-rose-100', btnClass: 'bg-rose-500', modalIconBg: 'bg-rose-500', tagBg: 'bg-rose-50', tagText: 'text-rose-700', tagBorder: 'border-rose-100', timeBg: 'bg-rose-50', timeText: 'text-rose-600' },
        kejia: { tableHeadBg: 'bg-indigo-50', tableHeadText: 'text-indigo-800', tableHeadBorder: 'border-indigo-100', btnClass: 'bg-indigo-500', modalIconBg: 'bg-indigo-500', tagBg: 'bg-indigo-50', tagText: 'text-indigo-700', tagBorder: 'border-indigo-100', timeBg: 'bg-indigo-50', timeText: 'text-indigo-600' },
        hubei: { tableHeadBg: 'bg-cyan-50', tableHeadText: 'text-cyan-800', tableHeadBorder: 'border-cyan-100', btnClass: 'bg-cyan-500', modalIconBg: 'bg-cyan-500', tagBg: 'bg-cyan-50', tagText: 'text-cyan-700', tagBorder: 'border-cyan-100', timeBg: 'bg-cyan-50', timeText: 'text-cyan-600' },
        dongbei: { tableHeadBg: 'bg-purple-50', tableHeadText: 'text-purple-800', tableHeadBorder: 'border-purple-100', btnClass: 'bg-purple-500', modalIconBg: 'bg-purple-500', tagBg: 'bg-purple-50', tagText: 'text-purple-700', tagBorder: 'border-purple-100', timeBg: 'bg-purple-50', timeText: 'text-purple-600' },
        beef: { tableHeadBg: 'bg-fuchsia-50', tableHeadText: 'text-fuchsia-800', tableHeadBorder: 'border-fuchsia-100', btnClass: 'bg-fuchsia-500', modalIconBg: 'bg-fuchsia-500', tagBg: 'bg-fuchsia-50', tagText: 'text-fuchsia-700', tagBorder: 'border-fuchsia-100', timeBg: 'bg-fuchsia-50', timeText: 'text-fuchsia-600' },
        pork: { tableHeadBg: 'bg-pink-50', tableHeadText: 'text-pink-800', tableHeadBorder: 'border-pink-100', btnClass: 'bg-pink-500', modalIconBg: 'bg-pink-500', tagBg: 'bg-pink-50', tagText: 'text-pink-700', tagBorder: 'border-pink-100', timeBg: 'bg-pink-50', timeText: 'text-pink-600' }
    };
    ...
    function switchCategory(category) {
        ...
        document.getElementById('table-wrapper').className = `table-container overflow-x-auto theme-${category}`;
    ...
    function handleSearch() {
        ...
        if (query === '') { renderTable(currentCategory); return; }
    ...
    function renderSearchResults(results, query) {
        document.getElementById('table-wrapper').className = `table-container overflow-x-auto`;
        document.getElementById('table-header').innerHTML = `
            <tr>
                <th class="w-16 text-center bg-gray-100 text-gray-700 border-b-2 border-gray-200">序号</th>
                <th class="w-48 bg-gray-100 text-gray-700 border-b-2 border-gray-200">菜品名称 <span class="text-sm font-normal text-blue-500">(搜索结果: ${results.length}道)</span></th>
                <th class="bg-gray-100 text-gray-700 border-b-2 border-gray-200">菜品简介</th>
                <th class="w-48 text-center bg-gray-100 text-gray-700 border-b-2 border-gray-200">操作与展示</th>
            </tr>
        `;
        ...
        if (results.length === 0) {
            recipeList.innerHTML = `<tr><td colspan="4" class="text-center py-10 text-gray-500 text-lg">全库未找到包含 "<span class="text-red-500">${query}</span>" 的相关菜谱，请尝试更换关键词。</td></tr>`;
            return;
        }
        ...
        results.forEach((item, index) => {
            ...
            tr.addEventListener('mouseenter', () => tr.style.backgroundColor = '#f3f4f6');
            tr.addEventListener('mouseleave', () => tr.style.backgroundColor = '');
            tr.onclick = () => openRecipeModal(category, originalIndex);
            
            const categoryName = document.querySelector(`option[value="${category}"]`).textContent.split(' ')[0];
            tr.innerHTML = `
                <td class="text-center font-medium text-gray-500">${index + 1}</td>
                <td class="font-bold text-gray-800 text-lg">${item.name}<span class="block text-xs font-normal text-gray-400 mt-1">所属分类：${categoryName}</span></td>
                <td class="text-gray-600">${item.desc}</td>
                <td class="text-center">
                    <img src="${item.image}" alt="${item.name}" class="dish-image" onerror="this.onerror=null; this.src='https://placehold.co/160x120?text=未找到图片';">
                    <div class="flex justify-center items-center gap-2 mt-2">
                        <span class="view-recipe-btn !mt-0 ${theme.btnClass}">查看做法</span>
                        <button onclick="deleteRecipe('${category}', ${originalIndex}, event)" class="px-3 py-1 bg-gray-100 text-gray-500 hover:bg-red-500 hover:text-white rounded-full text-xs font-medium transition-colors">删除</button>
                    </div>
                </td>
            `;
    ...
    function renderTable(category) {
        ...
        document.getElementById('table-header').innerHTML = `
            <tr>
                <th class="w-16 text-center ${theme.tableHeadBg} ${theme.tableHeadText} border-b-2 ${theme.tableHeadBorder}">序号</th>
                <th class="w-48 ${theme.tableHeadBg} ${theme.tableHeadText} border-b-2 ${theme.tableHeadBorder}">菜品名称</th>
                <th class="${theme.tableHeadBg} ${theme.tableHeadText} border-b-2 ${theme.tableHeadBorder}">菜品简介</th>
                <th class="w-48 text-center ${theme.tableHeadBg} ${theme.tableHeadText} border-b-2 ${theme.tableHeadBorder}">操作与展示</th>
            </tr>
        `;
        const recipeList = document.getElementById('recipe-list');
        recipeList.innerHTML = '';
        data.forEach((recipe, index) => {
            const tr = document.createElement('tr');
            tr.className = 'clickable-row';
            tr.onclick = () => openRecipeModal(category, index);
            tr.innerHTML = `
                <td class="text-center font-medium text-gray-500">${index + 1}</td>
                <td class="font-bold text-gray-800 text-lg">${recipe.name}</td>
                <td class="text-gray-600">${recipe.desc}</td>
                <td class="text-center">
                    <img src="${recipe.image}" alt="${recipe.name}" class="dish-image" onerror="this.onerror=null; this.src='https://placehold.co/160x120?text=图片加载失败';">
                    <div class="flex justify-center items-center gap-2 mt-2">
                        <span class="view-recipe-btn !mt-0 ${theme.btnClass}">查看做法</span>
                        <button onclick="deleteRecipe('${category}', ${index}, event)" class="px-3 py-1 bg-gray-100 text-gray-500 hover:bg-red-500 hover:text-white rounded-full text-xs font-medium transition-colors">删除</button>
                    </div>
                </td>
            `;
            recipeList.appendChild(tr);
    ...
    function openRecipeModal(category, index) {
        ...
        recipe.ingredients.forEach(item => {
            ...
            span.className = `${theme.tagBg} ${theme.tagText} border ${theme.tagBorder} px-3 py-1.5 rounded-full text-sm font-medium`;
        ...
        recipe.steps.forEach((step, i) => {
            ...
            div.innerHTML = `
                <span class="step-circle z-10 shadow-sm ${theme.modalIconBg}">${i + 1}</span>
                <div class="flex-1 bg-white border border-gray-100 p-4 rounded-xl shadow-sm ml-2">
                    <div class="flex justify-between items-center mb-2">
                        <span class="font-bold text-gray-800">第 ${i + 1} 步</span>
                        <span class="text-xs font-medium ${theme.timeText} ${theme.timeBg} px-2 py-1 rounded-full">耗时约 ${step.time}</span>
                    </div>
    ...
    function openLuckyDraw() {
        ...
        document.getElementById('slot-text').textContent = "点击下方按钮开始";
    ...
    function closeModalOnOutsideClick(event, modalId) {
        ...
        if (!modalContent.contains(event.target)) {
            if(!isDrawing) closeModal(modalId); // 抽奖进行中不可点击外侧关闭
        }
    }
